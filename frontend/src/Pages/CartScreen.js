...
const theme = createMuiTheme({
  palette: {
    ...
    secondary: {
      ...
      main: "rgb(255,40,40,0.8)",
    },
    tertiary: {
      main: "rgb(255,40,40,0.4)",
    },
  },
});
...
function BasicTable() {
  ...
  const PUT_URL = URLS.PUT;
  const [total, setTotal] = React.useState(0);
  const [products, setProducts] = React.useState([]);

  React.useEffect(() => {
    axios.get(ORDER_URL).then((response) => {
  ...
  const handleDelete = (i, id) => {
    ...
    setProducts(pro);
    axios.delete(PUT_URL + id).then((response) => console.log(response));
  };

  return (
    <ThemeProvider theme={theme}>
      <Appbar shops={products.length} />
      <Container>
                    ...
                    return (
                      <TableRow key={row.b_dresstype}>
                        ...
                        <TableCell align="right">
                          {row.product.e_arrival.toLowerCase() === "old"
                            ? "Rs." + row.product.f_discount
                            : row.product.d_price}
                        </TableCell>
                        <TableCell align="right">
                          <ButtonGroup>
                            <Button onClick={() => handleInc(i, row.id)}>
                              +
                            </Button>
                            <Button disabled> {row.quantity}</Button>
                            <Button
                              disabled={display}
                              onClick={() => handleDec(i, row.id)}
                            >
                              -
                            </Button>
                          </ButtonGroup>
